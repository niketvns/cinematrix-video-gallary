...
import {useGlobleLiked} from "../../contexts";
import {SpinnerLoader} from "../../components";
import {useNavigate} from "react-router-dom";

const Likes = () => {
    const {likedVideos, removeFromLike, isLikedLoading} = useGlobleLiked()
                ...
                likedVideos.length ?
                    <>
                        <div className="preview-image w-full backdrop-blur-0 bg-white/20 flex items-center md:flex-col gap-4 md:max-w-[300px] rounded-lg md:sticky top-16 md:max-h-[90vh] md:min-h-[89vh] md:p-2">
                            <img src={likedVideos[likedVideos.length-1].thumbnail} alt="preview" className={'min-w-[100px] w-[30vw] h-44 md:h-[40vh] bg-red-600 rounded-lg'}/>
                                ...
                                likedVideos.map(video => (
                                    <div key={video._id} className={'ind-video flex justify-start items-start gap-4 min-h-[220px] backdrop-sepia-0 bg-white/5 md:mr-4'}>
                                        ...
                                        <div className="contains h-[220px] flex flex-col justify-between flex-1 py-4">
                                            ...
                                            <button className={'bg-[#0EA5E9FF] p-2 rounded-lg max-w-[200px] inline'} onClick={()=>removeFromLike(video?._id)}>
                                                Remove <span className={'hidden md:inline'}>from Liked</span>
                                            </button>
                                        </div>
                                    </div>
                                ))
